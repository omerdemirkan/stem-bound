...
export default class AuthService {
    ...
    async userSignUp({
        ...
        switch (role) {
            ...
            case EUserRoles.SCHOOL_OFFICIAL:
                user = await this.schoolOfficialService.createSchoolOfficial(
                    userData
                );
    ...
    async getUserById({ id, role }: { id: Types.ObjectId; role: EUserRoles }) {
        ...
        switch (role) {
            case EUserRoles.INSTRUCTOR:
                ...
                break;
            case EUserRoles.SCHOOL_OFFICIAL:
                user = await this.schoolOfficialService.findSchoolOfficialById(
                    id
                );
                break;
            case EUserRoles.STUDENT:
    ...
    async getUserByEmail({ email, role }: { email: string; role: EUserRoles }) {
        ...
        switch (role) {
            case EUserRoles.INSTRUCTOR:
                user = await this.instructorService.findInstructorByEmail(
                    email
                );
                break;
            case EUserRoles.SCHOOL_OFFICIAL:
                user = await this.schoolOfficialService.findSchoolOfficialByEmail(
                    email
                );
                break;
            case EUserRoles.STUDENT:
